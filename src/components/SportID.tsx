...
import { connectWallet, getConnectionStatus, claimTitle } from '../utils/blockchain';
...
const SportID: React.FC<SportIDProps> = ({ initialProfile = mockSportProfile }) => {
  ...
  return (
    <div className="container">
      ...
      <div className="card profile-header">
        <img 
          src={profile.user.avatar || '/avatars/default.png'} 
        ...
        <div>
          ...
          {profile.user.walletAddress ? (
            <div className="badge">
              {`${profile.user.walletAddress.substring(0, 6)}...${profile.user.walletAddress.substring(profile.user.walletAddress.length - 4)}`}
            </div>
          ) : (
            <button 
      ...
      <div className="titles-grid">
        {profile.titles.map((title: TitleNFT) => (
          <div key={title.id} className="title-card">
            ...
            <div className={`badge badge-${title.rarity}`}>
              {title.rarity}
            </div>
            <div className="title-desc">
              {formatDate(title.dateEarned)}
            </div>
          </div>
        ))}
      </div>
          ...
          <div key={activity.id} className="activity-item">
            ...
            <div className="activity-details">
              <div className="activity-type">
                ...
                {activity.verified && (
                  <span className="badge badge-verified" style={{ marginLeft: '8px', fontSize: '0.7rem' }}>
                    Verified
                  </span>
                )}
              </div>
